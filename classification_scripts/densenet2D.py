...
class HeartClassification(Dataset):
    def __init__(self, X, y, transform=None, slice_axis=1):
        self.X = X  # List of image file paths
        self.y = y  # Corresponding labels
        self.transform = transform
        self.slice_axis = slice_axis  # Axis along which to extract 2D slices
        self.slice_counts = [nib.load(img).get_fdata().shape[self.slice_axis] for img in X]
    ...
    def __getitem__(self, index):
        # Determine which image and slice the index refers to
        cumulative_slices = 0
        for patient_idx, slice_count in enumerate(self.slice_counts):
            if cumulative_slices + slice_count > index:
                slice_idx = index - cumulative_slices
                break
            cumulative_slices += slice_count

        # Load the 3D image and extract the specified coronal slice
        image = nib.load(self.X[patient_idx]).get_fdata()
        slice_image = image[:, slice_idx, :]

        # Add a channel dimension (e.g., converting (H, W) to (C, H, W))
        slice_image = np.expand_dims(slice_image, axis=0)

        # Apply transformations (if any)
        if self.transform:
            slice_image = self.transform(slice_image)
        return slice_image, self.y[patient_idx]
