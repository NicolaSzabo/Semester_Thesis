...
class HeartClassification(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return multi_view_img, torch.tensor(self.labels[index], dtype=torch.long)
...
def train_and_validate(model, train_loader, val_loader, criterion, optimizer, epochs, device):
    for epoch in range(epochs):
        ...
        # Log multi-view slices as separate grayscale images
        if epoch % 5 == 0:  # Log every epoch
            ...
            for i in range(inputs_batch.size(0)):  # Loop over batch samples
                writer.add_image(f"Sample {i + 1}/Coronal", inputs_batch[i, 0], global_step=epoch, dataformats='HW')
                writer.add_image(f"Sample {i + 1}/Axial", inputs_batch[i, 1], global_step=epoch, dataformats='HW')
                writer.add_image(f"Sample {i + 1}/Sagittal", inputs_batch[i, 2], global_step=epoch, dataformats='HW')
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
    ...
    model = EfficientNet.from_pretrained(
        "efficientnet-b0",  # EfficientNet variant
        in_channels=3,  # 3 input channels for coronal, axial, sagittal views
        num_classes=2  # Number of output classes (healthy vs. pathological)
    ).to(device)
